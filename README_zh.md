...
- **检索、大篇幅吞吐与任务路由**：推荐使用极具性价比且性能强劲的最新“闪电级/小钢炮”模型。例如，使用 **Gemini 2.5 Flash**（拥有 100 万+ 超长上下文窗口，极度适合读取和总结整本科研文献或海量 PDF）或 **GPT-4o mini** 与 **Claude 3.5 Haiku**（适合高并发的子 Agent 编排、关键词提取和工作流路由）。
- **推理与写作阶段**：仅在最终撰写论文、进行深度科学推理和结构化诚信审计时，才切换至顶级的前沿模型（如 **Claude 3.5 Sonnet** 或 **GPT-4o**）。
