...
- **ステージ1（探索とフィルタリング）**：文献の初期解析、キーワード抽出、タスクのルーティングには、コスト効率が良くスループットの高い `deepseek-chat` (DeepSeek-V3) やローカルのオープンモデルを使用します。
- **ステージ2（統合と執筆）**：論文の最終執筆、深い推論、整合性監査には、`claude-3-5-sonnet` や `gpt-4o` などの最高性能モデルに切り替えます。
